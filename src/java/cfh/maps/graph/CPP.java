...
public class CPP {	
	...
	{	if( !defined[u][v] ) label[u][v] = new Vector<String>();
   ...
   //</tex><tex file="greedy.tex">
	...
	int findPath(int from, int ff[][]) // find a path between unbalanced vertices
	{	for( int i = 0; i < N; i++ )
			if( ff[from][i] > 0 ) return i;
		return NONE; 
	}
	...
	{	int v = startVertex;
		...
		{	int u = v;
			...
			{	int bridgeVertex = path[u][startVertex];
				if( ar[u][bridgeVertex] == 0 )
					break; // finished if bridge already used
				v = bridgeVertex;
				for( int i = 0; i < N; i++ ) // find an unused arc, using bridge last
					if( i != bridgeVertex && ar[u][i] > 0 )
					{	v = i;
						break;
					}
				ar[u][v]--; // decrement count of parallel arcs
				System.out.println("Take arc "+label[u][v].elementAt(ar[u][v])
